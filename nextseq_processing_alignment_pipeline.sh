...
echo "Enter '1' if you want to demultiplex this run?, if not (you already demultiplexed) enter '0', then press [ENTER]: ";
read demul;

echo "Enter '1' if you want to convert the fastqs->fasta, if not enter '0', then press [ENTER]: ";
read convert;

echo "Enter '1' if you want trim the fastqs->fasta, if not enter '0', then press [ENTER]: ";
read trunc;

#write info in script to logfile.
echo run number: "$run_number" demultiplexed: "$demul" converted: "$convert" truncated: "$trunc" >> /git/nextseq/processed/"$dir_name".logfile;
...
echo Skipping demultiplexing. >> /git/nextseq/processed/"$dir_name".logfile;
echo $d >> /git/nextseq/processed/"$dir_name".logfile;
